...
const { VecStoreFactory, FolderSync, getConfig, processors, WebCrawler } = require('../index');
const fs = require('fs');
...
async function crawl() {
  if (args.length < 3) {
    console.error('Usage: vecstore-js crawl <url> <output-dir> [--max-pages N] [--max-depth N]');
    process.exit(1);
  }
  ...
  const results = await crawler.crawlSite(url, outputDir);
...
Commands:
  ...
  crawl <url> <output-dir> [opts]              Crawl entire site with Playwright
