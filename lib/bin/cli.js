...
async function add() {
  if (args.length < 4) {
    console.error('Usage: vecstore-js add <db-path> <id> <text> [model]');
    process.exit(1);
  }
...
async function query() {
  if (args.length < 3) {
    console.error('Usage: vecstore-js query <db-path> <query-text> [topK] [model]');
    process.exit(1);
  }
...
async function syncFolder() {
  if (args.length < 3) {
    console.error('Usage: vecstore-js sync <db-path> <folder-path> [model] [--extensions .pdf,.txt] [--no-recursive]');
    process.exit(1);
  }
...
async function crawl() {
  if (args.length < 3) {
    console.error('Usage: vecstore-js crawl <url> <output-dir> [--max-pages N] [--max-depth N] [--db-path path] [--model name]');
    process.exit(1);
  }
...
async function update() {
  if (args.length < 2) {
    console.error('Usage: vecstore-js update <db-path> [model]');
    process.exit(1);
  }
...
function help() {
  console.log(`
vecstore-js - Local vector database CLI

Usage:
  npx vecstore-js <command> [options]
...
Examples:
  npx vecstore-js init ./mydb.db
  npx vecstore-js add ./mydb.db doc1 "Hello world"
  npx vecstore-js query ./mydb.db "greeting" 5
  npx vecstore-js sync ./mydb.db ./docs
  npx vecstore-js sync ./mydb.db ./docs --extensions .pdf,.docx
  npx vecstore-js crawl https://example.com ./crawled --max-pages=50
  npx vecstore-js crawl https://example.com ./crawled --db-path=./db.db
  npx vecstore-js update ./mydb.db
  npx vecstore-js processors

Default model: all-minilm (via Ollama)
