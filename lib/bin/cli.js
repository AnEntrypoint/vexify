...
async function crawl() {
  ...
  const url = args[1];
  const outputDir = args[2] || `./${new URL(url).hostname.replace(/^www\./, '')}`;
  ...
  const onPageCrawled = async (page) => {
    ...
    const processor = processors.getProcessor(ext);
    ...
    const documents = await processor.process(page.path, { url: page.url });
...
Crawl Options:
  ...
  --model name               Embedding model (default: embeddinggemma)
