...
class GoogleDriveCrawler {
  ...
  async executeWorkPlan(vecStore, plan, state) {
    ...
    for (const { file, isUpdate } of workItems) {
      ...
      try {
        ...
        console.log(`  [${processed + 1}/${Math.min(workItems.length, limit)}] ${action}: ${file.name}`);
  ...
  async crawl(folderId = 'root', vecStore) {
    ...
    console.log(`✓ Scanned ${allFiles.length} files in ${scanTime}s`);

    const existingFiles = await this.getExistingFiles(vecStore);
    console.log(`  Existing in DB: ${Object.keys(existingFiles).length}`);

    const plan = await this.buildWorkPlan(vecStore, allFiles, existingFiles);

    console.log(`\n📋 Work Plan:`);
    console.log(`  New: ${plan.toAdd.length}`);
    console.log(`  Updated: ${plan.toUpdate.length}`);
