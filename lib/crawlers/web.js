  ...
  async crawlSite(startUrl, outputDir, vecStore = null, onPageCallback = null) {
    ...
    while (crashCount < maxCrashRetries) {
      try {
        ...
        console.log(`  Crash count: ${crashCount}/${maxCrashRetries}`);

        if (this.visited.size >= this.maxPages || this.queue.length === 0) {
          console.log(`  Crawl complete (visited: ${this.visited.size}, queued: ${this.queue.length})`);
          break;
        }

        console.log(`  Automatically restarting in ${backoffDelay / 1000}s...\n`);
    ...
    if (crashCount >= maxCrashRetries) {
      console.log(`  ⚠ Maximum crash retries (${maxCrashRetries}) reached. Stopping.`);
    }
    ...
    while (this.queue.length > 0 && this.visited.size < this.maxPages) {
            ...
            try {
              ...
              if (error.message.includes('Target page, context or browser has been closed')) {
                if (retry < this.maxRetries) {
                  console.log(`  ⚠ Browser closed, restarting... (retry ${retry + 1}/${this.maxRetries})`);
                  try {
                    await browser.close();
                  } catch (e) {}
      ...
      if (onPageCallback) {
        ...
        if (validItems.length > 0) {
          await Promise.all(validItems.map(item => onPageCallback(item)));
        }
      }
