...
class CodeCrawler {
  ...
  async processFile(filePath, vecStore, onPageCallback) {
    try {
      ...
      console.log(`  ✓ Indexed: ${relativePath} (${language})`);
  ...
  async crawl(vecStore = null, onPageCallback = null) {
    ...
    console.log(`   Max depth: ${this.maxDepth}, Max file size: ${Math.round(this.maxFileSize / 1024)}KB`);

    const ignoreManager = new IgnoreManager(this.rootPath, this.customIgnorePatterns);
    const ignoreStats = ignoreManager.getIgnoreStats();
    console.log(`   Ignore patterns: ${ignoreStats.patterns} loaded`);
    ...
    console.log(`   Files indexed: ${this.stats.indexedFiles}`);
