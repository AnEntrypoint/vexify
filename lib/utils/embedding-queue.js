...
class EmbeddingQueue {
  ...
  async drain() {
    ...
    console.log(`[Drain] Starting: ${this.queue.length} queued, ${this.activeBatches} active, maxConcurrent: ${this.maxConcurrent}`);
    ...
    while (this.queue.length > 0 || this.activeBatches > 0) {
      ...
      if (iterations % 100 === 0) {
        console.log(`[Drain] ${iterations/10}s: ${this.queue.length} queued, ${this.activeBatches} active`);
      }
    ...
    console.log(`[Drain] Complete: ${this.queue.length} queued, ${this.activeBatches} active`);
  }
}
